...
export class ExhibitService {
  ...
  async createQuestionnaire(
    exhibitQuestionaireData: Questionnairedto,
    ...
    exhibitQuestionaire.editorID = exhibitQuestionaireData.editor.userId;
    ...
    const updatedQuestionnaire =
      await this.exhibitQuestionaireRepository.save(exhibitQuestionaire);
  ...
  async updateQuestionnaire(
    ...
    await this.exhibitQuestionaireRepository.update(
      id,
      exhibitQuestionaireData,
    );
    const updatedQuestionnaire =
      await this.exhibitQuestionaireRepository.findOne({ where: { id: id } });
  ...
  async createQuestion(exhibitQuestionData: QuestionDto): Promise<Question> {
    const exhibitQuestion =
      this.exhibitQuestionRepository.create(exhibitQuestionData);
  ...
  async getQuestionById(id: string): Promise<Question | null> {
    const exhibitQuestion = await this.exhibitQuestionRepository.findOne({
      where: { id: id },
    });
  ...
  async updateQuestion(
    id: string,
    exhibitQuestionData: QuestionDto,
  ): Promise<Question | null> {
    await this.exhibitQuestionRepository.update(id, exhibitQuestionData);
    const updatedQuestion = await this.exhibitQuestionRepository.findOne({
      where: { id: id },
    });
